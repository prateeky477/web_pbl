...
require('config.php');
$dalog = $_GET['id'];
$result = mysqli_query($conn, "SELECT * FROM `data`,`data1`,`data2` WHERE `data`.`login_id`=$dalog AND `data1`.`login_id`=$dalog AND `data2`.`login_id`=$dalog");
            ...
            <div class="card-body">
              ...
              <div class="row">
                <div class="col-sm-12">
                  <hr>
                  <br>
                  <a class="btn btn-info " target="__blank" href="search.php" style="background-color:white; color:black; border: 2px solid #4CAF50 ; text-decoration:none" ;>Search</a>
                </div>
              </div>
            </div>
          </div>
